...
def onReceive(msg):
	...
	"""
	update = {'msg': msg+'(parsed)'}
	sub = {}
	if "postfix/smtp[" in msg:
	    r = re.search('to=<(.*)>, relay=(.*)\[.*status=(\w+) (.*)', msg)
            # we sent something here
            if r:
		if len(r.groups()) == 4:
                     sub.update({'recipient': r.groups()[0]})
                     sub.update({'relay': r.groups()[1]})
                     sub.update({'status': r.groups()[2]})
                     sub.update({'statusmsg': r.groups()[3]})
		     update.update({'$!': sub})
                else:
                     update.update({'failed': True})
            else:
                update.update({'failed': True})
